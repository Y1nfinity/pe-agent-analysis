...
def bar_paired_unmatched_spend_by_vintage(vintage_overview: pd.DataFrame,
    ...
    ax.set_title("Coverage by Vintage: Paired vs Unmatched Entry Spend")
    ax.set_ylabel("Entry Spend (USD MN)")
    ax.yaxis.set_major_formatter(money_fmt())
    ax.legend(frameon=False)
...
def stacked_paired_unmatched_fraction_spend_by_vintage(breakdown: pd.DataFrame,
                                                       unmatched_v: pd.DataFrame,
                                                       out: Path):
    """
    Stacked bar: per vintage, fraction of ENTRY spend that is paired (exited) vs unmatched.
    Denominator: total entry spend per vintage (paired + unmatched).
    Numerators:
      - paired: sum of breakdown.entry_spend_for_exits
      - unmatched: unmatched_v.unmatched_entry_spend
    """
    # Paired spend per vintage
    paired = (breakdown.groupby("EntryYear", as_index=False)["entry_spend_for_exits"]
              .sum()
              .rename(columns={"entry_spend_for_exits": "paired_entry_spend"}))

    # Unmatched spend per vintage
    un = unmatched_v[["EntryYear", "unmatched_entry_spend"]].copy()

    # Merge
    merged = (paired.merge(un, on="EntryYear", how="outer")
                    .fillna(0)
                    .sort_values("EntryYear"))
    merged["EntryYear"] = merged["EntryYear"].astype(int)

    # Fractions
    denom_safe = (merged["paired_entry_spend"] + merged["unmatched_entry_spend"]).replace(0, np.nan)
    merged["frac_paired_spend"] = merged["paired_entry_spend"] / denom_safe
    merged["frac_unmatched_spend"] = merged["unmatched_entry_spend"] / denom_safe

    # Plot stacked fractions
    fig, ax = plt.subplots(figsize=(14, 5))
    x = np.arange(len(merged))
    ax.bar(x, merged["frac_paired_spend"], label="Paired spend")
    ax.bar(x, merged["frac_unmatched_spend"], bottom=merged["frac_paired_spend"],
           label="Unmatched spend")

    ax.set_title("Coverage by Vintage: Fractions of Entry Spend (Paired vs Unmatched)")
    ax.set_ylabel("Fraction of Vintage Entry Spend")
    ax.yaxis.set_major_formatter(pct_fmt())
    _apply_year_ticks(ax, merged["EntryYear"].tolist(), max_target=20, rotation=0)
    ax.legend(frameon=False)
