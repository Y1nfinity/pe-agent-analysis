...
def plot_completion_bars(summary: pd.DataFrame,
    ...
    V_total = df["V_entry_total"].fillna(0).values
    V_exit  = df["V_entry_exited"].fillna(0).values

    frac_N = df["frac_exited_N"].fillna(0).values
    frac_V = df["frac_exited_value"].fillna(0).values

    fig, axN = plt.subplots(figsize=(14, 6))
    axV = axN.twinx()
    ...
    for xn, xv, fn, fv, nE, vE in zip(x_blue, x_red, frac_N, frac_V, N_exit, V_exit):
        if nE > 0:
            axN.text(xn, nE * 0.97, f"{fn*100:.0f}%",
                     ha="center", va="top", fontsize=7, color="white")
        if vE > 0:
            axV.text(xv, vE * 0.97, f"{fv*100:.0f}%",
                     ha="center", va="top", fontsize=7, color="white")

    # =======================
    # AXES & LABELS
    # =======================
    axN.set_ylabel("Number of Deals", color="blue")
    axV.set_ylabel("Entry Value ($m)", color="red")

    axN.tick_params(axis='y', colors='blue')
    axV.tick_params(axis='y', colors='red')

    axN.set_xticks(list(x))
    ...
    axN.legend(h1 + h2, l1 + l2, loc="upper left")
